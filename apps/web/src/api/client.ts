...
export const api = {
  ...
  licenseStatus: (signal?: AbortSignal) => request<LicenseStatusDto>("/licenses/current", { signal }),
  licenses: (signal?: AbortSignal) => getList<LicenseRecordDto>("/licenses", undefined, signal),
  installLicense: (envelope: LicenseEnvelopeDto) => request<LicenseRecordDto>("/licenses/install", { method: "POST", body: JSON.stringify({ envelope }), headers: { "Idempotency-Key": crypto.randomUUID() } }),
  adminTenants: (params?: Record<string, QueryValue>, signal?: AbortSignal) => getList<TenantSummaryDto>("/admin/tenants", params, signal),
  createTenant: (payload: { tenant_id: string; display_name: string }) => request<TenantDto>("/admin/tenants", { method: "POST", body: JSON.stringify(payload) }),
  updateTenantState: (id: string, state: "ACTIVE" | "SUSPENDED") => request<TenantDto>(`/admin/tenants/${encodeURIComponent(id)}`, { method: "PATCH", body: JSON.stringify({ state }) }),
