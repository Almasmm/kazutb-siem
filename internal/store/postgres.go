...
func (p *Postgres) ResetTenant(ctx context.Context, tenantID string) error {
	...
	defer func() { _ = tx.Rollback(ctx) }()
	for _, table := range []string{"audit_entries", "incidents", "alerts", "findings", "security_events", "collectors", "hunt_executions", "saved_hunts", "detection_correlation_emissions", "detection_correlation_observations", "detection_rule_versions", "tenant_retention_policies"} {
