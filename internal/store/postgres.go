...
func (p *Postgres) ResetTenant(ctx context.Context, tenantID string) error {
	...
	defer func() { _ = tx.Rollback(ctx) }()
	for _, table := range []string{"report_runs", "parser_versions", "ai_soc_decisions", "ai_soc_requests", "ai_soc_policies", "ueba_anomalies", "ueba_volume_windows", "ueba_entity_baselines", "service_accounts", "audit_entries", "evidence_custody_entries", "evidence_items", "case_incidents", "cases", "incidents", "alerts", "findings", "security_events", "entity_relation_observations", "entity_event_observations", "entity_relations", "security_entities", "agent_credentials", "agent_enrollment_tokens", "collectors", "hunt_executions", "saved_hunts", "soar_connector_tests", "soar_connector_rate_windows", "soar_connectors", "soar_approval_decisions", "soar_approvals", "soar_action_attempts", "soar_node_executions", "soar_executions", "soar_playbook_versions", "soar_playbooks", "threat_intel_matches", "threat_intel_indicator_sources", "threat_intel_indicators", "threat_intel_feeds", "detection_correlation_emissions", "detection_correlation_observations", "detection_rule_versions", "tenant_retention_policies"} {
